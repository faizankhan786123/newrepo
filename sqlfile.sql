public static String sendSMS(IFormReference iform, String stage, String data) {
		try{
			...
			String smsLang = "EN";
			//end
			CSR_BT.mLogger.debug("inside sendSMScall Card_No :" + Card_No);
			//String path = System.getProperty("user.dir") + File.separatorChar + "folder name" + File.separatorChar + 
			//		"RAKBank_SMS_Temp" + File.separatorChar + "CSR_BT_"+stage+".html";
			...
			CSR_BT.mLogger.debug("SMS Triggred successfuly if value of status is 1-------------STATUS = " + status);
			if(status==1)
			return "true";
			}
		}catch(Exception ex) {
