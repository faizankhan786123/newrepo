USE [rakcas]
GO
/****** Object:  StoredProcedure [dbo].[NG_TS_DOC_REMINDER_MAIL_PROC]    Script Date: 8/11/2025 1:37:18 PM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	DECLARE @Infobip_Dynamic_Tags nvarchar(200) = '';
	DECLARE @DynamicValues nvarchar(200) = '';
	DECLARE @HEADER_CARD_NO nvarchar(200) = '';
	...
	BEGIN  
      print 'ProcessInstanceID'+@ProcessInstanceID
			...
			@Infobip_SMS_isActive = Infobip_SMS_isActive,@infobip_Alert_id = Infobip_Alert_ID,@Infobip_Dynamic_Tags = Infobip_Dynamic_Tags from NG_TS_TEMPLATE_MAPPING_MASTER WITH(NOLOCK) where SERVICE_NAME = 'Card Dispute' and TEMPLATE_ID = '39' and ISACTIVE = 'Y'
			print('Alert id from 39 template '+@infobip_Alert_id)
			...
			print 'MAil'+@MAIL_FROM
				...
				begin
					if(dbo.CPMSDateDiffExcludingWeekends(@entryDATETIME,GETDATE())=1 OR dbo.CPMSDateDiffExcludingWeekends(@entryDATETIME,GETDATE())=1)
					print 'SMS'
				...
				print 'inside sms date25'
				  SELECT @CIF_ID = CIF_ID ,@HEADER_CARD_NO = HEADER_CARD_NO
					 ...
					 -- Get last 4 digits of the card number
                        SET @DynamicValues = CONCAT(RIGHT(@HEADER_CARD_NO, 4),'~',@WINAME)
					...
					begin
						print 'SMS-25'
						IF (@Infobip_SMS_isActive = 'N')
				print 'if Infobip_SMS_isActive is N '+@Infobip_SMS_isActive;
                  ...
                  ELSE IF (@Infobip_SMS_isActive = 'Y')
                 BEGIN
				 print 'if Infobip_SMS_isActive is Y '+@Infobip_SMS_isActive;
